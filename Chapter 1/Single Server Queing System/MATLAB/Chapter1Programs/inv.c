...
/* khonsari: to run 
gcc inv.c lcgrand.c -l(m, ..)
e.g. we need link to math.h
then it become 
gcc inv.c lcgrand.c -lm
then 
./a.out
then inv.out is created
look at inv.out
*/
...
int   amount, bigs, initial_inv_level, inv_level, next_event_type, num_events,
      num_months, num_values_demand, smalls, counter = 0;
...
//khonsari add  int before main()  /* Main function. */
...
void order_arrival(void)  /* Order arrival event function. */
{
    ...
    inv_level += amount;
    printf("in function order arival the inv_level is: %d. and counter is: %d\n", inv_level, counter);
...
void demand(void)  /* Demand event function. */
{
    ...
    printf("in function demand the inv_level is: %d. and counter is: %d\n", inv_level, counter);
